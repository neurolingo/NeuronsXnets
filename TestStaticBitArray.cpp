...
void print(nlg::StaticBitArray<N,T> const &bitarr, std::string_view msg)
{
  ...
  for (T block : bitarr)
  {
    std::bitset<nlg::StaticBitArray<N,T>::bits_per_block>  bset(block);
...
void TestBitArrayFastRotate(int num_tests)
{
  ...
  for (int i=0; i < num_tests; i++)
  {
    ...
    {
      nlg::StaticBitArray<num_bits,block_type> bitarr_r1;
      nlg::StaticBitArray<num_bits,block_type> bitarr_r2;
...
void TestMaskCreation()
{
  ...
  nlg::StaticBitArray<num_bits,block_type> spikes_1;
  nlg::StaticBitArray<num_bits,block_type> spikes_2;
  ...
  nlg::StaticBitArray<num_bits,block_type> shift_spikes;
